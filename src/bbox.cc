...
kdBox::kdBox() : bEmpty(true)
{
}

kdBox::kdBox(glm::vec3 bMin, glm::vec3 bMax)
        : bmin(bMin), bmax(bMax), bEmpty(false), dirty(true)
{
}

bool kdBox::intersects(const kdBox& target) const
{
...
bool kdBox::intersects(const glm::dvec3& point) const
{
...
bool kdBox::intersect(glm::dvec3 position, glm::dvec3 direction, double& tMin, double& tMax) const
{
	/*
 	 * Kay/Kajiya algorithm.
	 */
	glm::dvec3 R0 = position;
	glm::dvec3 Rd = direction;
...
void kdBox::operator=(const kdBox& target)
{
	bmin    = target.bmin;
	bmax    = target.bmax;
	bArea   = target.bArea;
...
void kdBox::merge(const kdBox& bBox)
{
	if (bBox.bEmpty)
		return;
