...
// C++ 11 Strset_nesting_level Literal
// See http://en.cppreference.com/w/cpp/language/strset_nesting_level_literal
...
void main()
{
    ...
    world_coord = vec4(vertex_position.x, vertex_position.y, vertex_position.z, 1);
}
)zzz";
...
void main()
{
    ...
    for (n = 0; n < gl_in.length(); n++) {
        ...
        world_coordinate = vec4(world_coord[n].x, world_coord[n].y, world_coord[n].z, 1);
        EmitVertex();
    }
    EndPrimitive();
}
...
void main()
{
    vec4 color = clamp(abs(normal), 0.0, 1.0);
...
in vec4 normal;
in vec4 light_direction;
in vec4 world_coordinate;
out vec4 fragment_color;
void main()
{
    if (mod(floor(world_coordinate.x / 3), 2) == 1) {
        if (mod(floor(world_coordinate.z / 3), 2) == 1) {
        ...
        const std::vector<glm::uvec3>& indices)
{
    ...
    for (glm::vec4 vertex : vertices) {
        fInput << "v " << vertex[0] << " " << vertex[1] << " " << vertex[2] << "\n";
    }
 
    for (glm::uvec3 triangle : indices) {
        fInput << "f " << ((int) triangle[0]) + 1 << " " << ((int) triangle[1]) + 1 << " " << ((int) triangle[2]) + 1 << "\n";
    }
            ...
            int mods)
{
    ...
    if (key == GLFW_KEY_ESCAPE && action == GLFW_PRESS)
        ...
        GLint* vertexSize = new GLint;
        glGetBufferParameteriv(GL_ARRAY_BUFFER, GL_BUFFER_SIZE, vertexSize);
        GLint* facesSize = new GLint;
        glGetBufferParameteriv(GL_ELEMENT_ARRAY_BUFFER, GL_BUFFER_SIZE, facesSize);
 
        glm::vec4* vertices = new glm::vec4[*vertexSize / (sizeof(float) * 4)];
        glm::uvec3* faces = new glm::uvec3[*facesSize / (sizeof(uint32_t) * 3)];
        glGetBufferSubData(GL_ARRAY_BUFFER, 0, *vertexSize, vertices);
        glGetBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0, *facesSize, faces);
 
        std::vector<glm::vec4> vertexList;
        glm::vec4* vListPtr = vertices;
        int numVertices = *vertexSize / (sizeof(float) * 4);
        for (int i = 0; i < numVertices; i++) {
            vertexList.push_back(*(vListPtr + i));
        }
 
        std::vector<glm::uvec3> indexList;
        glm::uvec3* iListPtr = faces;
        int numFaces = *facesSize / (sizeof(uint32_t) * 3);
        for (int i = 0; i < numFaces; i++) {
            indexList.push_back(*(iListPtr + i));
        }
 
        SaveObj("geometry.obj", vertexList, indexList);
 
    } else if (key == GLFW_KEY_W && action != GLFW_RELEASE) {
        g_camera.forward_backward(true);
    } else if (key == GLFW_KEY_S && action != GLFW_RELEASE) {
        g_camera.forward_backward(false);
    } else if (key == GLFW_KEY_A && action != GLFW_RELEASE) {
        g_camera.strafe(false);
    } else if (key == GLFW_KEY_D && action != GLFW_RELEASE) {
        g_camera.strafe(true);
    } else if (key == GLFW_KEY_LEFT && action != GLFW_RELEASE) {
        g_camera.roll(false);
    } else if (key == GLFW_KEY_RIGHT && action != GLFW_RELEASE) {
        g_camera.roll(true);
    } else if (key == GLFW_KEY_DOWN && action != GLFW_RELEASE) {
        g_camera.up_down(false);
    } else if (key == GLFW_KEY_UP && action != GLFW_RELEASE) {
        g_camera.up_down(true);
    ...
    if (key == GLFW_KEY_0 && action != GLFW_RELEASE) {
        // FIXME: Change nestset_nesting_level level of g_menger
...
MousePosCallback(GLFWwindow* window, double mouse_x, double mouse_y)
{
    ...
    if (g_current_button == GLFW_MOUSE_BUTTON_LEFT) {
        g_camera.drag(glm::vec3(mouse_x, mouse_y, 0));
...
void generatePlane(std::vector<glm::uvec3> &faces) {
...
int main(int argc, char* argv[])
{
    ...
    glfwSwapInterval(1);
    const GLubyte* renderer = glGetString(GL_RENDERER);  // get renderer strset_nesting_level
    const GLubyte* version = glGetString(GL_VERSION);    // version as a strset_nesting_level
    ...
    // NOTE: We do not send anythset_nesting_level right now, we just describe it to OpenGL.
    ...
    //center
    locations.push_back(glm::vec4(0, -2, 0, 1));
 
    // triangle coords top right top left bottom left bottom right
    locations.push_back(glm::vec4(10000, -2, 10000, 1));
    locations.push_back(glm::vec4(-10000, -2, 10000, 1));
    locations.push_back(glm::vec4(-10000, -2, -10000, 1));
    locations.push_back(glm::vec4(10000, -2, -10000, 1));
    ...
    // NOTE: We do not send anythset_nesting_level right now, we just describe it to OpenGL.
    CHECK_GL_ERROR(glBufferData(GL_ARRAY_BUFFER,
                                sizeof(float) * locations.size() * 4, locations.data(),
                                GL_STATIC_DRAW));
    CHECK_GL_ERROR(glVertexAttribPointer(0, 4, GL_FLOAT, GL_FALSE, 0, 0));
    CHECK_GL_ERROR(glEnableVertexAttribArray(0));
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        CHECK_GL_ERROR(glDrawElements(GL_TRIANGLES, locations.size() * 3, GL_UNSIGNED_INT, 0));
 
        //  indicated by VAO.
 
        //CHECK_GL_ERROR(glUseProgram(floor_program_id));
 
        //CHECK_GL_ERROR(glDrawElements(GL_TRIANGLES, floor_face.size() * 3, GL_UNSIGNED_INT, 0));
